...
function buildRouterSystemPrompt() {
  return [
    ...
    "Use cafeteria_lookup for cafeteria menus, meals, 학식, 식단, 조식, 중식, 석식.",
    ...
    "For searchQuery, extract only the core lookup target. Examples: '학교 안에 변전 시설 있나?' -> '변전소'; '초막 교회 위치' -> '초막교회'; '오늘 점심 학식' -> null; '전주역에서 가는 법' -> null.",
