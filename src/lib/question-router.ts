...
function buildRouterSystemPrompt() {
  return [
    ...
    "Allowed intent values: campus_place_lookup, cafeteria_lookup, department_lookup, graduate_school_lookup, transport_lookup, professor_lookup, official_rag, unknown.",
    ...
    "Use graduate_school_lookup for graduate schools, 대학원 existence, 대학원 종류, 대학원 입학, or graduate admissions.",
    ...
    "For searchQuery, extract only the core lookup target. Examples: '학교 안에 변전 시설 있나?' -> '변전소'; '초막 교회 위치' -> '초막교회'; '학교에 식당은 어디에 있어?' -> '식당'; '전주대학교 학과 전부 알려줘' -> null; '전주대학교에는 대학원이 있어?' -> null; '오늘 점심 학식' -> null; '전주역에서 가는 법' -> null.",
